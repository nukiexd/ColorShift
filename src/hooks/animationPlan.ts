...
export type AnimationStepType = 'swap' | 'clear' | 'fall' | 'refill' | 'shuffle';
...
export function buildMoveAnimationPlan(session: GameSession, from: Coord, to: Coord, options: AnimationPlanOptions): MoveAnimationPlan {
  ...
  if (!resolution.accepted) return { accepted: false, resolution, steps: [], randomState: session.randomState, tileIdCounter: session.tileIdCounter };
  ...
  return { accepted: true, resolution, steps, randomState: random.getState(), tileIdCounter: tileIds.getCounter() };
}
