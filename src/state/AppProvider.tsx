...
export function AppProvider({ children, seedFactory }: AppProviderProps) {
  ...
  const nextSessionOrdinal = useRef(1);
  ...
  const makeSession = useCallback(() => createSession(seedFactory ? seedFactory() : nextSeed.current++, nextSessionOrdinal.current++), [seedFactory]);
  ...
  const updateNickname = useCallback((nickname: string) => {
    ...
    commitState({ ...stateRef.current, profile: { ...stateRef.current.profile, nickname: normalized } });
    return true;
  }, [commitState]);
